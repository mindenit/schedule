...
const swipeAnimation = ref<"swipe-left" | "swipe-right" | null>(null)

const { direction, isSwiping } = useSwipe(dayViewEl)

watch(isSwiping, (swiping) => {
	if (swiping && direction.value === "right") {
		swipeAnimation.value = "swipe-right"
	...
	if (swiping && direction.value === "left") {
		swipeAnimation.value = "swipe-left"
...
<template>
	<div ref="dayView" class="flex h-full overflow-hidden md:rounded-lg">
		<Transition
			:enter-active-class="swipeAnimation === 'swipe-left' ? 'animate-swipe-left' : 'animate-swipe-right'"
      :leave-active-class="swipeAnimation === 'swipe-left' ? 'animate-swipe-left' : 'animate-swipe-right'"
		>
			<div v-if="!swipeAnimation" class="flex flex-1 flex-col overflow-hidden">
